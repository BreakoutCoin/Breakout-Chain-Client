...
// NOTE: Breakout's txnouttype has only the standard script types, so the
//       Stealth qPoS (Junaeth), claim, and feeless output-type flags are
//       omitted here. Bit positions are preserved for the flags that remain
//       so any pre-existing serialized txflags values stay meaningful.
enum ExploreTxFlags
{
    // tx types
