...
void HDTxInfo::GetAccountBalanceChange(std::map<int, int64_t>& mapChangeRet) const
{
    mapChangeRet.clear();
    BOOST_FOREACH(const AddrInOutInfo& addrinout, addrinouts)
    {
        mapChangeRet[addrinout.GetColor()] += addrinout.GetBalanceChange();
    }
}
                      ...
                      Object& objRet) const
{
    ...
    // per-color account balance change (Breakout is multi-color)
    std::map<int, int64_t> mapChange;
    GetAccountBalanceChange(mapChange);
    Array aryChange;
    for (std::map<int, int64_t>::const_iterator it = mapChange.begin();
         it != mapChange.end(); ++it)
    {
        Object objChange;
        objChange.push_back(Pair("color", (boost::int64_t)it->first));
        objChange.push_back(Pair("change", ValueFromAmount(it->second, it->first)));
        aryChange.push_back(objChange);
    }
    objRet.push_back(Pair("account_balance_change", aryChange));
