...
int InOutInfo::GetColor() const
{
    if (IsInput())
    {
        return inout.input.color;
    }
    else
    {
        return inout.output.color;
    }
}
...
void InOutInfo::AsUniqueJSON(Object& objRet) const
{
    ...
    if (IsInput())
    {
        ...
        amount = ValueFromAmount(inout.input.amount, inout.input.color);
    }
    else
    {
        n_label = "vout";
        ...
        amount = ValueFromAmount(inout.output.amount, inout.output.color);
    }
                       ...
                       Object& objInOut) const
{
    ...
    if (IsInput())
    {
        ...
        n_label = "vin";
        n = inout.input.vin;
        balance = ValueFromAmount(inout.input.balance, inout.input.color);
        other_txid = inout.input.prev_txid.GetHex();
        other_n = inout.input.prev_vout;
        amount = ValueFromAmount(inout.input.amount, inout.input.color);
    }
    else
    {
        ...
        n_label = "vout";
        n = inout.output.vout;
        balance = ValueFromAmount(inout.output.balance, inout.output.color);
        other_txid = inout.output.next_txid.GetHex();
        other_n = inout.output.next_vin;
        amount = ValueFromAmount(inout.output.amount, inout.output.color);
    }
