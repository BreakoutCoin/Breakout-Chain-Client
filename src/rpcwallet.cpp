...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <breakoutaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress <breakoutaddress> <amount> [comment] [comment-to] [tx-comment] [product-id] [narration]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <breakoutaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <breakoutaddress> <signature> <message>\n"
...
Value gethand(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "gethand [minconf=1]\n");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <tobreakoutaddress> <amount> [minconf=1] [comment] [comment-to] [tx-comment] [product-id] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "[tx-comment] max size is " + strprintf("%d", MAX_TX_COMMENT_LEN) + "\n"
            "[product-id] is a positive integer\n"
	    "[narration] is a short (<=24 char) note"
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (nColor == (int) BREAKOUT_COLOR_NONE)
        {
...
Value validatepubkey(const Array& params, bool fHelp)
{
    ...
    if (isValid)
    {
        CTxDestination dest = address.Get();
        string currentAddress = address.ToString();
        if (address.nColor == BREAKOUT_COLOR_NONE)
        {
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Breakout Stealth Address."));
        return result;
    };
...
Value clearwallettransactions(const Array& params, bool fHelp)
{
    ...
    Object result;
    string strRemoved = strprintf("Removed %u transactions.", nTx);
    result.push_back(Pair("complete", strRemoved));
