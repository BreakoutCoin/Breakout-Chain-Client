...
bool CKey::SetSecret(const CSecret& vchSecret, EC_KEY *pkey, bool fCompressed)
{
    EC_KEY_free(pkey);
    pkey = EC_KEY_new_by_curve_name(NID_secp256k1);
    if (pkey == NULL)
...
// TODO: this is a hack to get around the switch of key systems
//          it needs to be put in a class
CKey PubKeyWithSecret(CSecret vchSecret, CPubKey &cpkT)
{
    CECKey ceck;
    CKey ckey;
    EC_KEY *pkey = ceck.GetPKey();
    ckey.SetSecret(vchSecret, pkey, true);
    CPrivKey cpk;
    ceck.GetPrivKey(cpk, true);
    ckey.SetPrivKey(cpk, true);

    ceck.GetPubKey(cpkT, true);
