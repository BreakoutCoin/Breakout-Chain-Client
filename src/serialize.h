...
inline ethash::hash256 to_hash256(const uint256& hash)
{
    ...
    memcpy(result.bytes, hash.cbegin(), 32);
    return result;
}

// uint256 to_uint256(const std::string& hex) defined in ethash/helpers.hpp

inline uint256 to_uint256(const ethash::hash256& hash)
{
    uint256 result;
    memcpy(result.begin(), hash.bytes, 32);
    return result;
}


inline std::string GetHex(const unsigned char* pch, unsigned int length)
{
    std::string result(length * 2, '\0');
    for (unsigned int i = 0; i < length; ++i)
    {
        snprintf(&result[i * 2], 3, "%02x", pch[length - i - 1]);
    }
    return result;
}
...
public:
    ...
    CDataStream(const valtype& vchIn, int nTypeIn, int nVersionIn) : vch((char*)&vchIn.begin()[0], (char*)&vchIn.end()[0])
    {
        Init(nTypeIn, nVersionIn);
    }
    ...
    CDataStream& read(char* pch, int nSize)
    {
        ...
        if (nReadPosNext >= vch.size())
        {
            if (nReadPosNext > vch.size())
            {
                ...
                printf("==\nData is:\n%s\n==\n",
                       GetHex((const unsigned char*)pch, nSize).c_str());
                memset(pch, 0, nSize);
